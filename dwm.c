...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - stw - x) > bh) {
		if (m->sel) {
            if(!highlightline) {
                ...
                // Don't switch theme if highlightline should be used
                drw_setscheme(drw, scheme[SchemeNorm]);
            }
    ...
    // Draw the highlightline if requested
    if(m == selmon && highlightline) {
        drw_setscheme(drw, scheme[SchemeSel]);
        drw_rect(drw, 0, 0, m->ww, highlightlineh, 1, 1);
    }
